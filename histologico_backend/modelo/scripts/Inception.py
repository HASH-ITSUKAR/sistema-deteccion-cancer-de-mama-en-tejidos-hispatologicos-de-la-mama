...
ruta_inception = os.path.join(directorio_actual, "modelos", "Inception.pth")

# Inicialización del modelo InceptionNeXt Small
modelo = create_model("inception_next_small", pretrained=False, num_classes=2)
modelo.load_state_dict(torch.load(ruta_inception, map_location=device))
modelo.to(device).eval()

# Diccionarios globales para capturar activaciones y gradientes de la última etapa convolucional
activaciones = {}
gradientes = {}
...
# Registrar los hooks en la última etapa (stage) de la red convolucional
